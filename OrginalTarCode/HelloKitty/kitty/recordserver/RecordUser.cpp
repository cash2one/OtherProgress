...
bool RecordUser::syncBaseMemDB()
{
	...
	if (!handle->setInt("charinfo",acctype, account, charid)) return false;

	if(!handle->set("rolebaseinfo", this->charid, "nickname",nickname)) return false;

	//启动时初始化会话登录状态，理论上没有必要
	if (!handle->setInt("sessionlogin", this->charid, "state", 0)) return false;
...
bool RecordUser::refreshSaveBase(const CMD::RECORD::t_WriteUser_SceneRecord *rev)
{
	...
	unsigned int retcode = RecordService::dbConnPool->exeUpdate(handle, "t_charbase", record_charbase, (BYTE *)(&rev->charbase), where);
	RecordService::dbConnPool->putHandle(handle);

	if (1 == retcode)
	{
		Fir::logger->trace("[角色读写],charid=%lu,保存档案成功 retcode=%u", rev->charid, retcode);
	}
	else if (((unsigned int)-1) == retcode)
	{
		Fir::logger->error("[角色读写],charid=%lu,0, 保存档案失败 retcode=%u", rev->charid, retcode);
		return false;
	}
