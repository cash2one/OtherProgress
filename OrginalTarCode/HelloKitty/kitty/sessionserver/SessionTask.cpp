...
bool SessionTask::msgParseStruct(const CMD::t_NullCmd *ptNullCmd, const DWORD nCmdLen)
{
	...
	{
		this->gate_user_cmdparse(ptNullCmd, nCmdLen);
		return true;
	}
	else
	{

		if (this->gate_cmd_dispatcher.dispatch(this, ptNullCmd, nCmdLen))
			return true;

		if (this->scene_cmd_dispatcher.dispatch(this, ptNullCmd, nCmdLen))
			return true;
	}
...
bool SessionTask::gate_user_cmdparse(const CMD::t_NullCmd *ptNullCmd, const unsigned int nCmdLen)
{
	...
	SessionUser *u = SessionUserManager::getMe().getUserByID(rev->dwID);
	if (!u)
	{
	...
	if (!this->user_dispatcher.dispatch(u, cmd, cmdlen))
        {
                CMD::stRetErrorOperationUserCmd_SC send;
                send.cmd_id = cmd->_id;
                u->sendCmd(&send, sizeof(send));
        }
