		...
		case PARA_STARTUP_SERVERENTRY_NOTIFYOTHER:
			{
                ...
                *(BYTE*)buffer = STRUCT_TYPE;
                bcopy(ptCmd,buffer+sizeof(BYTE),nCmdLen);
				return sendCmd(buffer, nCmdLen+sizeof(BYTE));
			}
...
bool zSubNetService::init()
{
    ...
    *(BYTE*)buffer = STRUCT_TYPE;
	...
	t_Startup_Request *tCmd = (t_Startup_Request*)(buffer+sizeof(BYTE));
    constructInPlace(tCmd);
	tCmd->wdServerType = wdServerType;
	strncpy(tCmd->pstrIP, pstrIP, sizeof(tCmd->pstrIP)-1);
    len += sizeof(t_Startup_Request);
   
	if (!superClient->sendCmd(buffer,len)) 
	{
...
bool zSubNetService::validate()
{
    ...
    *(BYTE*)buffer = STRUCT_TYPE;
	...
	CMD::SUPER::t_Startup_OK *tCmd = (CMD::SUPER::t_Startup_OK*)(buffer+len);
	constructInPlace(tCmd);
    tCmd->wdServerID = wdServerID;
    len += sizeof(CMD::SUPER::t_Startup_OK);
	return superClient->sendCmd(buffer, len);
}
