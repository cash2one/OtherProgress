...
namespace tbx {
	...
	template <typename table_type, typename tbx_type>
		class table {
			public:
				typedef std::function<bool (const tbx_type &)> for_each_func;

			public:
				table() {}
				virtual ~table() {}

				inline const tbx_type &get(unsigned int id) const {
					static tbx_type tmp;
					if (!id)
					{
				...
				inline const tbx_type* get_base(unsigned int id) const {
					auto it = _tbx_map.find(id);
					if (it != _tbx_map.end())
						return it->second;
					return NULL;
				...
				void for_each(const tbx::table<table_type, tbx_type>::for_each_func &func) const {
					for (int i = 0; i < _proto.datas_size(); ++i)
						if (!func(_proto.datas(i)))
							break;
				}
				typedef typename std::unordered_map<unsigned int, const tbx_type *>::const_iterator _tbx_iter;
			...
			private:
				table_type _proto;
				std::unordered_map<unsigned int, const tbx_type *>	_tbx_map;
		};
	...
	class mgr : public Fir::Singleton<mgr> {
		...
		private:
			...
			template <typename table_type, typename tbx_type>
				void load_table(const std::string &name, tbx::table<table_type, tbx_type> &tb, const ReadTbxBufFunc &func) {
