...
bool GYListManager::verifyVer(DWORD zoneid, DWORD verify_client_version, HelloKittyMsgData::LoginFailReason &retcode)
{
	...
	else if (ret->zoneGameVersion && ret->zoneGameVersion != verify_client_version && verify_client_version != 501938573)
	{
		Fir::logger->error("客户端连接没有通过版本号验证 , 客户端版本号:%d,服务器版本号%d",verify_client_version,ret->zoneGameVersion);
