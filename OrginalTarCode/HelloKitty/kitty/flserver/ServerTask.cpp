...
int ServerTask::verifyConn()
{
	...
	if (retcode > 0)
	{
        ...
        if (CMD_LOGIN == ptCmd->cmd && PARA_LOGIN == ptCmd->para && ServerACL::getMe().check(/*ptCmd->strIP*/getIP(), ptCmd->port, gameZone, name))
        {
		...
		case PARA_FL_GYLIST:
			{
				...
				Fir::logger->debug("gateway更新状态:zoneid=%u,serverid=%u,ip=%s,port=%u,rolenum=%u,state=%u,version=%u,nettype=%u", gameZone.zone, ptCmd->wdServerID, ptCmd->pstrIP, ptCmd->wdPort, ptCmd->wdNumOnline, ptCmd->state, ptCmd->zoneGameVersion, ptCmd->wdNetType);
				...
				CMD::Slave::t_GYList_Master cmd;
				cmd.gameZone = gameZone;
				cmd.wdServerID = ptCmd->wdServerID;
				bcopy(ptCmd->pstrIP, cmd.pstrIP, sizeof(cmd.pstrIP));
				cmd.wdPort = ptCmd->wdPort;
				cmd.wdNumOnline = ptCmd->wdNumOnline;
				cmd.state = ptCmd->state;
				cmd.zoneGameVersion = ptCmd->zoneGameVersion;
				cmd.wdNetType = ptCmd->wdNetType;
...
bool ServerTask::msgParse_session(const CMD::t_NullCmd *ptNullCmd, const unsigned int nCmdLen)
{
	...
	switch(ptNullCmd->para)
	{
		case PARA_SESSION_NEWSESSION:
			{
				...
				Fir::logger->debug("broadcastNewSession, loginTempID=%d", ptCmd->session.loginTempID);
		...
		case PARA_SESSION_REG_ROLE_COUNT:
			{
				t_RegRoleCount_Session* ptCmd = (t_RegRoleCount_Session*)ptNullCmd;	
