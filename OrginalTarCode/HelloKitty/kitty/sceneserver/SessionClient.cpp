...
bool SessionClient::msgParseStruct(const CMD::t_NullCmd *ptNullCmd, const DWORD nCmdLen)
{
	...
	if (cmdCountSwitch) analysis.add(ptNullCmd->cmd, ptNullCmd->para, nCmdLen);

	
	if (this->user_login_dispatcher.dispatch(this, ptNullCmd, nCmdLen))
	{
		return true;
	}
	if (this->session_client_dispatcher.dispatch(this, ptNullCmd, nCmdLen))
	{
		return true;
	}
