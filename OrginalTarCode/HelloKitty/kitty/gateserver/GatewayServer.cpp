...
bool GatewayService::init()
{
	...
	Fir::logger->info("服务器版本号:%d",verify_client_version);
...
bool GatewayService::initLogger()
{
	...
	snprintf(srv , sizeof(srv)-1, "GS[%d]",getServerID());
	bill_logger = FIR_NEW zLogger(srv);
	...
	if ("" != Fir::global["billlogfile"])
	{
		...
		bill_logger->addLocalFileLog(srv);
...
bool GatewayService::initLoginLogger()
{
	...
	{   
		...
		{   
			strncpy(sub,tok,sizeof(sub));
			bzero(tok , strlen(tok));
			snprintf(srv + strlen(srv) , sizeof(srv)-strlen(srv), ".gateway%d",getServerID());
			strncat(srv,sub,sizeof(srv) - strlen(srv));
		}   
		else
		{   
	...
	if (CMD_GATEWAY == ptNullCmd->cmd)
	{
		switch(ptNullCmd->para)
		{
			case PARA_SUPER2GATE_KICK_OUT_USER:
				{
					t_Super2GateKickOutUser* rev = (t_Super2GateKickOutUser*)ptNullCmd;
					// 踢玩家下线
...
void GatewayService::final()
{
	...
	if(sessionClient)
	{
		Fir::logger->trace("[缷载跟踪]: cmd:%u para:%u len:%u usercmd:%u userpara:%u userlen:%u", 
				sessionClient->last_cmd, sessionClient->last_para, sessionClient->last_cmd_len,
				sessionClient->last_user_cmd, sessionClient->last_user_para, sessionClient->last_user_cmd_len);
