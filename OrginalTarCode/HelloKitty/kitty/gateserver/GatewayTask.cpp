...
int GatewayTask::verifyConn()
{
	...
	if (retcode > 0)
	{
		while(true)
		{
            ...
            {
                return msgParseStruct((CMD::t_NullCmd*)pstrCmd,nCmdLen);
            }
 ...
 * \return 转发是否成功
 */
bool GatewayTask::forwardScene(const CMD::NullCmd *ptNullCmd, const unsigned int nCmdLen)
{
	if(m_pUser && m_pUser->scene)
	{
		...
		m_pUser->scene->sendCmd(buf,sizeof(CMD::SCENE::t_Scene_ForwardScene)+nCmdLen);
		//Fir::logger->debug("转发%ld的消息到%ld场景",pUser->id,pUser->sceneTempID);
		return true;
	}
	return false;
}
 ...
 * \return 转发是否成功
 */
bool GatewayTask::forwardSession(const CMD::NullCmd *ptNullCmd, const unsigned int nCmdLen)
{
	if(m_pUser)
	{
		BYTE buf[zSocket::MAX_DATASIZE];
		...
		//Fir::logger->debug("转发%ld的消息到Session服务器",pUser->id,pUser->sceneTempID);
		return true;
	}
	return false;
}
 ...
 * \return 解析是否成功
 */
bool GatewayTask::msgParse_Time(const CMD::NullCmd *ptNullCmd, const unsigned int nCmdLen)
{
	using namespace CMD;
...
bool GatewayTask::msgParseProto(const BYTE *data, const DWORD nCmdLen)
{
    ...
    if(ret)
    {
        zRTime time;
        time.now();
        GatewayTask::lastCmdTime = time.msecs();
    }
    SAFE_DELETE(message);
	return ret;
}
...
{ 
	...
	using namespace CMD;
	const NullCmd *ptNullCmd = (const NullCmd *)ptNull;
	...
	if (m_pUser)
	{
		if (this->forward_cmd_dispatch(m_pUser, (CMD::NullCmd*)ptNullCmd, nCmdLen))
		{
			zRTime time;
			time.now();
			GatewayTask::lastCmdTime = time.msecs();
			if (ptNullCmd && ptNullCmd->_id >= 3000)
			{
#ifdef _ALL_SUPER_GM
				Fir::logger->debug("[接收消息时间]消息号%u,%llu",ptNullCmd->_id,GatewayTask::lastCmdTime);
#endif
			}
			return true;
		}
...
bool GatewayTask::sendCmd(const void *pstrCmd, int nCmdLen)
{
	...
	if (ptNullCmd && ptNullCmd->_id >= 3000)
	{
		...
		time.now();
#ifdef _ALL_SUPER_GM
		unsigned long long sec = time.msecs();
		unsigned long long left = sec > GatewayTask::lastCmdTime ? sec - GatewayTask::lastCmdTime : 0;
		Fir::logger->debug("[消息统计]消息号:%u,距上一次接收消息相隔%llu毫秒,收到消息时间:%llu,发送消息时间:%llu",ptNullCmd->_id,left,GatewayTask::lastCmdTime,sec);
#endif
	}
#endif
...
bool GatewayTask::forward_cmd_dispatch(GateUser* u, const CMD::NullCmd* ptNullCmd, const unsigned int nCmdLen)
{
	using namespace CMD::ID;
	if (!u || !u->isPlayState())
		return false;

	const DWORD  &cmd = ptNullCmd->_id;

	if (cmd == 65535 || cmd == 65534 || (cmd >= CMD::ID::STMING2SCENE && cmd < CMD::ID::STMAXG2SCENE))
	{
		//转发用户消息到场景，没有使用dispatcher, 提高效率
		return forwardScene(ptNullCmd, nCmdLen);
	}

	if (cmd == 65533 || cmd == 65532
			|| (cmd >= CMD::ID::STMING2SESSION && cmd <= CMD::ID::STMAXG2SESSION)) 
	{
		return forwardSession(ptNullCmd, nCmdLen);
	}
	if (cmd >= CMD::ID::STMING2SUPER && cmd <= STMAXG2SUPER)
	{//转发用户消息到super
	}
