...
class SceneTaskManager : public Singleton<SceneTaskManager>
{
	...
	public:
		...
		void execEvery();
        //向网关广播消息
        bool broadcastUserCmdToGateway(const QWORD charid,const void *pstrCmd, int nCmdLen);
        //向固定网关发的固定角色发送消息
        bool broadcastUserCmdToGateway(const DWORD serverID,const QWORD charid,const void *pstrCmd, int nCmdLen);
	private:
