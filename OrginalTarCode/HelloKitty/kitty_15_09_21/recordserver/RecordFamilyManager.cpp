...
BYTE RecordFamilyM::createFamily(const CMD::RECORD::FamilyBase &base)
{
    BYTE ret = 0;
    do{
        zMemDB* handle = zMemDBPool::getMe().getMemDBHandle(base.m_charid % MAX_MEM_DB+1);
        if(!handle)
        {
            ret = 1;
            break;
...
bool  RecordFamilyM::readFamily(QWORD familyID)
{
    ...
    zMemDB* reshandle = zMemDBPool::getMe().getMemDBHandle(familyID % MAX_MEM_DB+1);
    if (reshandle==NULL) 
    {
        return false;
    }
...
{
    zMemDB* handle = zMemDBPool::getMe().getMemDBHandle(familyID % MAX_MEM_DB+1);
    if (handle==NULL) 
    {
...
bool RecordFamilyM::checkCalRank(bool forbit)
{
    ...
    if(!forbit)
    {
        ...
        for(auto iter = setfamilyID.begin(); iter != setfamilyID.end(); iter++)
        {
            ...
            if(RecordTimeTick::isSameday(base.m_createtime))
            {
                continue;
            }
            ...
            {
                zMemDB* handle3 = zMemDBPool::getMe().getMemDBHandle(*it % MAX_MEM_DB+1);
                if (!handle3) 
                {    
                    continue;
                }    
                if (handle3->getBin("charbase",*it,"charbase",(char*)&charbase) == 0)
                {
                    continue;
                }
                totallevel += charbase.level;
            }
            mapRank.insert(std::make_pair(totallevel,*iter));
            mapFamily[*iter] = base;
        }
        DWORD rankTatol =0;
...
bool RecordFamilyM::update(const CMD::RECORD::FamilyBase &base)
{
    ...
    zMemDB* redishandle = zMemDBPool::getMe().getMemDBHandle(base.m_familyID % MAX_MEM_DB+1);
    if (redishandle==NULL)
    {
...
{
    zMemDB* redishandle = zMemDBPool::getMe().getMemDBHandle(familyID % MAX_MEM_DB+1);
    if (redishandle==NULL)
    {
...
bool RecordFamilyM::addMemberForCreate(QWORD playerID,QWORD familyID)
{
    ...
    return  addMember(playerID,familyID);
