...
void logMessage(const char *data,const DWORD nCmdLen,const bool sendFlg)
{
    ...
    if(nCmdLen - sizeof(BYTE)-sizeof(DWORD)-nameLen < 0)
    {
        int lens = nCmdLen - sizeof(BYTE)-sizeof(DWORD)-nameLen;
        Fir::logger->debug("proto [%s警告消息长度](%s,%d)",sendFlg ? "发送" : "接收",messageName,lens);
        return;
    }
    logProtoMessage(messageName,nCmdLen-sizeof(BYTE)-sizeof(DWORD)-nameLen,sendFlg);
    return;
}
