...
bool LoginTask::requireLogin(const HelloKittyMsgData::ReqLogin * message)
{
	...
	if(account.empty())
	{
        Fir::logger->error("[客户端登录_2]:客户端请求网关失败(账号为空,%s)",message->account().c_str());
	...
	{
        Fir::logger->debug("[客户端登录_2]:客户端请求网关成功(验证版本,%u,%u,%s)",gameZone.zone,verify_client_version,message->account().c_str());
	}
	else
	{
        Fir::logger->error("[客户端登录_2]:客户端请求网关失败(验证版本,%u,%u,%s)",gameZone.zone,verify_client_version,message->account().c_str());
