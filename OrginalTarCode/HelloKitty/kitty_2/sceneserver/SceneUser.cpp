 ...
 *
 */
void SceneUser::setupBinaryArchive(const HelloKittyMsgData::Serialize& binary)
{
	//在线时长
...
bool SceneUser::reg_unreg(const BYTE retcode)
{
	...
	send.charid = this->charid;
	send.accid = this->accid;
	send.retcode = retcode;

    std::string ret;
    encodeMessage(&send,sizeof(send),ret);
	return sessionClient->sendCmd(ret.c_str(),ret.size());
}

void SceneUser::offline()
{
	CMD::RECORD::t_freshUser_SceneRecord fresh;
    fresh.charid = this->charid;
	fresh.accid  = this->accid;
	fresh.scene_id = 0;
    
    std::string ret;
    encodeMessage(&fresh,sizeof(fresh),ret);
	recordClient->sendCmd(ret.c_str(),ret.size());
...
bool SceneUser::online(std::string phone_uuid,SceneTask* _gate)
{
	...
	//----所有上线初始化，放在该行以上------
	//通知会话，场景上线成功
    CMD::SESSION::t_regUserSuccess_SceneSession send;
	send.charid = this->charid;
	send.accid = this->accid;
	send.gate_id = gate->getID();
...
void SceneUser::onlineInit()
{
    ...
    {
        this->cm.setCounter(IS_FIRST_LOGIN,1);
        this->_trade.initTradeInfo();
        save();
    }	
    HelloKittyMsgData::FlushUserInfo send;
    ...
    userBaseInfo->set_gold(this->charbase.gold);
    userBaseInfo->set_gem(this->charbase.gem);
    userBaseInfo->set_exp(0);
