...
bool ServerTask::msgParseZoneCmd(const CMD::ALLZONE::AllZoneNull *zoneNull,const DWORD nCmdLen)
{
    ...
    switch(zoneNull->para)
    {
        case PARA_ALL_ZONE_SUPER_FORWARD_SCENE:
            {   
                t_AllZone2Super_Forward2Scene *ptCmd = (t_AllZone2Super_Forward2Scene*)zoneNull;

                std::string ret;
                encodeMessage(ptCmd,ptCmd->getSize(),ret);
				AllZoneClientManager::getMe().broadcastOne(ret.c_str(),ret.size());
                return true;
            }
            break;
    }
...
bool ServerTask::msgParseSuperCmd(const CMD::SUPER::SuperServerNull *superNull,const DWORD nCmdLen)
{
    ...
    switch(superNull->para)
    {
        case PARA_FORWARD_ALLZONE_FROM_SCENE:
            {   
                t_ForwardAllZoneFromScene *ptCmd = (t_ForwardAllZoneFromScene *)superNull;
