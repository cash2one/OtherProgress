...
#ifdef _DEBUG_WCX
	Fir::logger->debug("[帐号登陆] logintype:%u, platAccontu:%s",task->acctype, task->account.c_str());
#endif
	if (!account_map.insert(std::make_pair(std::pair<WORD, std::string>(task->acctype, task->account),task)).second)
	{
		rwlock.unlock();
		return false;
	}
...
bool GatewayTaskManager::uniqueRemove(GatewayTask *task)
{
	...
	auto it = account_map.find(std::pair<WORD,std::string>(task->acctype, task->account));
