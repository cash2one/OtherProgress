...
class GateUserManager : public Singleton<GateUserManager>
{
	...
	public:
		...
		//plat_account map
		GateUser * getUserAccount(WORD platform, const std::string& account);
		bool addUserAccount(GateUser* user);
		void removeUserAccount(WORD platform, const std::string& account);
	...
	private:
		...
		std::map<std::pair<WORD,std::string>, GateUser*> account_map;	//所有帐号登陆的玩家
