...
bool LoginManager::broadcastNewSession(const DWORD loginTempID, const t_NewLoginSession &session)
{
	...
	if (it != loginTaskSet.end())
	{
        HelloKittyMsgData::LoginSuccessReturn loginSuccess;
		LoginTask *task = it->second;

		if (!task->checkACCNAME(session.account))
		{
            Fir::logger->error("%s 串号，服务器繁忙，断开连接 %s, %s", __PRETTY_FUNCTION__, session.account, task->getACCNAME());
			task->LoginReturnMsg("登录超时",HelloKittyMsgData::TimeOut);
			return false;
		}

		Fir::logger->info("登陆成功:%s, %u, %s, %u, %u, %u, %u, %s, %u", (NULL==task)?"0.0.0.0":task->getIP(),session.loginTempID, session.pstrIP, session.wdPort, session.gameZone.id, session.gameZone.game,session.gameZone.zone,session.account, session.acctype);
...
bool LoginManager::CallBackLogin(const DWORD loginTempID, const HelloKittyMsgData::ClientRequestLogin *message)
{
	zRWLock_scope_rdlock scope_rdlock(rwlock);
	LoginTaskHashmap_iterator it = loginTaskSet.find(loginTempID);
	if (it != loginTaskSet.end())
