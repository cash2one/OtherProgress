...
class ZoneTask;
typedef ProtoDispatcher<ZoneTask> ZoneCmdDispatcher;
...
class ZoneTask : public zTCPTask , public MessageQueue
{
    ...
    private:
        //处理zone消息枢纽
        bool msgParseZoneCmd(const CMD::ALLZONE::AllZoneNull *zoneNull,const DWORD nCmdLen);
        //处理场景发过来的proto消息
        bool msgSceneParseProto(const BYTE *data, const DWORD nCmdLen);
    public:

        //场景转发过来的消息处理
        static ZoneCmdDispatcher s_zone_scene_cmd_dispatch;
