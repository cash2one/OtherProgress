...
class RecordTask : public zTCPTask
{
    ...
    private:
		...
		DWORD create_role_inner(std::string phone_uuid,WORD acctype, std::string account, std::string nickname, std::string password,BYTE bySex, DWORD flatid);
        //初始化角色信息
        bool initCharBase(Record &record);    
    private:
