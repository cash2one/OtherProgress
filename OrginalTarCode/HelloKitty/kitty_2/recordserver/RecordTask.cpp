...
bool RecordTask::initCharBase(Record &record)
{
     const pb::Conf_t_newRoleAttr *confBase = tbx::newRoleAttr().get_base(1);
     if(!confBase)
     {
         Fir::logger->debug("RecordTask::initCharBase 初始化角色属性找不到对应的excel表数据");
         return false;
     }
 ...
 //    record.put("f_exp",0);
     //金币
     record.put("f_gold",confBase->roleAttr->gold());
     //人民币(钻石)
     record.put("f_gem",confBase->roleAttr->gem());
     //仓库大小
